...
describe('ngx-eq', () => {
  ...
  it('should apply the ngx-eq-sm class when the element is small', () => {
    ...
    expect(page.getFirstElement().getAttribute('class')).toContain('test-row ngx-eq ngx-eq-sm');
  });

  it('should apply the ngx-eq-md class when the element is medium', () => {
    page.setMediumBrowserSize();
    expect(page.getFirstElement().getAttribute('class')).toContain('test-row ngx-eq ngx-eq-sm ngx-eq-md');
  });
