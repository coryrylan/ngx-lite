...
export class NgxInputTagComponent implements ControlValueAccessor {
  ...
  handleClick(event: MouseEvent) {
    if (!this.inputElement.nativeElement.contains(event.target) && this.inputElement.nativeElement.value) {
  ...
  addTag(tag: string) {
    ...
    if (!tagIsEmpty && invalidTagLength) {
      this.tagError = { message: `Tag length cannot exceed ${maxTagLength} characters` };
    }
    ...
    if (exceedsMaxNumberOfTags) {
      ...
      this.tagError = { message: `Cannot exceed ${this.maxNumberOfTags} tag${plural}` };
    }

    if (!tagIsEmpty && !invalidTagLength && !duplicateTag && !exceedsMaxNumberOfTags) {
      this.tagError = null;
  ...
  addTagEvent(event: KeyboardEvent) {
    ...
    if (event.keyCode === KeyCodes.Backspace && this.prevTagInput.length === 0) {
      this._value.pop();
      this.setCurrentNumberOfTags();
    } else if (event.keyCode === KeyCodes.Enter || event.keyCode === KeyCodes.Comma || event.keyCode === KeyCodes.Tab) {
